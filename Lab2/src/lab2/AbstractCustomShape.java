...
public abstract class AbstractCustomShape
{
	 ...
	 *            Objet graphique qui est utilisé pour dessiner
	 */
	protected abstract void drawRectangle(Graphics pGraphic, int drawX,
			int drawY);
	...
	/*
	 * CODE EMPRUNTÉ : Les fonctions suivantes ont été inspirées par l'exemple
	 * sur le Comparator dans le chapitre 4 du livre de Horstmann. L'exemple a
	 * été trouvé sur le site ci-dessous dans un fichier zip que j'ai
	 * téléchargé. http://www.horstmann.com/design_and_patterns.html
	 */
	...
	{
		if (sequenceNumber < shape.sequenceNumber)
			return -1;
		if (sequenceNumber > shape.sequenceNumber)
			return 1;
		return 0;
	...
	{
		if (defaultOrder < shape.defaultOrder)
			return -1;
		if (defaultOrder > shape.defaultOrder)
			return 1;
		return 0;
